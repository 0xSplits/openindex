...
export function renderApiFunction(options: {
  ...
  const errorIds = resolveErrorData({
    dataLookup,
    id: `openindex!${module}.${displayName}.ErrorType:type`, // UPDATE ME
  })
  if (errorIds.size)
    content.push(
      renderErrors({
...
function renderImports(options: { entrypoint: string; module: string }) {
  ...
  const content = [
    ...
    [
      ':::code-group',
      '```ts [Named]',
      `import { ${module} } from 'openindex${entrypoint ? `/${entrypoint}` : ''}'`, // UPDATE ME
      '```',
      '```ts [Entrypoint]',
      `import * as ${module} from 'openindex/${entrypoint ? `${entrypoint}/` : ''}${module}'`, // UPDATE ME
      '```',
      ':::',
    ].join('\n'),
  ]
...
function renderErrors(options: {
  ...
  const typeRegex = /^openindex!(?<type>.+):type/ // UPDATE ME
  const errorType =
...
function resolveErrorData(options: {
  ...
  for (const reference of errorData.references) {
    const nextErrorData =
      dataLookup[`openindex!${reference.text}:type`] ?? // UPDATE ME
...
function getTypeLink(options: {
  ...
  const data = (() => {
    ...
    // if (dataLookup[`openindex!${type.type}:type`]) // UPDATE ME
    //   return dataLookup[`openindex!${type.type}:type`] // UPDATE ME
...
function expandInlineType(options: {
  ...
  const expandRegex = /^openindex!(?<type>.+)(_2):type/ // UPDATE ME
  if (
  ...
  } else if (dataLookup[`openindex!${type.type}:type`]) return type.type // UPDATE ME
  return type.type
}
